...
class JavaBottomNavigationFragment : Fragment() {
    ...
    fun initViews(mView: View) {
        ...
        codeView.setCode(getString(R.string.bottom_navigation_java), "java")
        val diffsAdapter = CodeWithDiffsAdapter(mContext)


        Objects.requireNonNull(codeView.getOptions())
            ?.withLanguage("java")
            ?.code = getString(R.string.bottom_navigation_java)
