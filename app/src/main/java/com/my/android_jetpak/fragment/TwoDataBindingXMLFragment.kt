...
class TwoDataBindingXMLFragment : Fragment() {
    ...
    fun initViews(mView: View) {
        ...
        codeView.setCode(getString(R.string.two_way_data_binding_xml), "xml")
        val diffsAdapter = CodeWithDiffsAdapter(mContext)


        Objects.requireNonNull(codeView.getOptions())
            ?.withLanguage("xml")
            ?.code = getString(R.string.two_way_data_binding_xml)
